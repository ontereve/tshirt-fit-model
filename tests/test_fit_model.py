...
def test_bulk_projection_increases_bulk_fields():
    ...
    if "TorsoLength" in body:
        assert (
            bulk["TorsoLength"] == body["TorsoLength"]
        ), "TorsoLength should stay the same unless modeling a growth spurt"
...
def test_score_fit_with_missing_fields():
    ...
    result = score_fit(body, shirt)
    assert isinstance(
        result["FitScore"], (int, float)
    ), "Should still return numeric FitScore with partial data"
...
def test_tags_and_rationale_present():
    ...
    assert "Rationale" in result and isinstance(
...
def test_bulk_confidence_lower():
    ...
    assert (
        bulk["Confidence"] <= normal["Confidence"]
    ), "Bulk confidence should not exceed normal profile confidence"
